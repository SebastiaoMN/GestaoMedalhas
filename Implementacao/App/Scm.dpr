...
begin
 ...
 //Antonio 2021
  if Not DirectoryExists('c:\sis\sc') then
    if DirectoryExists('c:\sis') then
       CreateDir('c:\sis\sc')
    else
      if CreateDir('c:\sis') then
        CreateDir('c:\sis\sc');
//  DM_SCM.DMSCM.DBSCM.Session.NetFileDir := ExtractFilePath(Application.EXEName);
  DM_SCM.DMSCM.DBSCM.Session.NetFileDir := 'c:\sis\sc';
  DM_SCM.DMSCM.DBSCM.Session.PrivateDir := 'c:\sis\sc';
